...
El enriquecimiento se implementa sobre `buscarDetallePorNumero` del gateway documental. La operacion unitaria permite enriquecer manualmente un documento especifico, y el proceso por lote reutiliza esa misma operacion para documentos pendientes. Esto evita duplicar reglas cuando una consulta interactiva futura necesite completar metadata al mostrar historicos de documentos oficiales o vinculados. El Worker limita su lote mediante la tabla `Configuracion_TemasEnriquecimientoMetadataDocumento`: un documento es elegible cuando esta vinculado a un expediente cuya trata pertenece a un tema habilitado. La prioridad de esa configuracion define el orden en que los temas completan el lote y la ausencia de temas habilitados detiene el proceso sin consumir cuota.
